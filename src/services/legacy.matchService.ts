...
export class LeagueOfLegendService {
  ...
  getMatchesByGameByNameAndTag = async (summonerName: string, tagLine: string, queueId: keyof typeof RIOT_QUEUE_IDS, count = 20) => {
    const riotAccount: RiotAccount = await this.riotService.getAccountByNameTagLine(summonerName, tagLine);
  ...
  updateMatchesByGameByNameAndTag = async (summonerName: string, tagLine: string, queueId: keyof typeof RIOT_QUEUE_IDS, count = 20) => {
    const riotAccount: RiotAccount = await this.riotService.getAccountByNameTagLine(summonerName, tagLine);
